...
function giveMembershipStatus(password, id, status, cb) {
  let passwordHash = process.env.MEMBER_PASSWORD;
  if (status === "admin") passwordHash = process.env.ADMIN_PASSWORD;
  bcrypt.compare(password, passwordHash, (err, success) => {
    if (err) {
      cb(err, null);
    }
    if (success) {
      User.findByIdAndUpdate(id, { membership_status: status }, {}, (err) => {
        if (err) {
          cb(err, null);
        }
        cb(null, true);
...
exports.membership_post = function (req, res, next) {
  ...
  giveMembershipStatus(
    ...
    (err, success) => {
      ...
      if (!success)
        giveMembershipStatus(req.body.password, req.user.id, "admin", (err) => {
          if (err) {
            return next(err);
          }
          res.redirect("/");
